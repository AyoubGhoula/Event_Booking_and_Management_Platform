...
useEffect(() => {
    const fetchEvents = async () => {
        try {
            ...
            const response = await axios.get('http://localhost:8000/api/participant/past-reservations', {
                headers: {
                    Authorization: `Bearer ${token}`
                }
            });
...
{notification && (
   ...
   <li>
      <motion.a href="#" className="flex items-center p-2  rounded-lg text-white bg-gray-700 group mb-7" data-tooltip-id="tooltip-3"
   ...
   <div className="flex items-center justify-center h-48 mb-4 rounded">
            <div className="mx-auto mt-6 w-full px-6 lg:mx-0 lg:max-w-none max-w-3xl">
              <div className="mb-6 items-end justify-between text-white flex">
                <div>
                  <h1 className="text-9xl p-2 ms:text-2xl font-bold bg-gradient-to-r from-cyan-500 via-sky-500 to-blue-500 bg-clip-text text-transparent">Past Events</h1>
                  <p className="text-gray-400 mt-7 pl-10 text-2xl bg-gradient-to-r from-fuchsia-500 to-pink-500 bg-clip-text text-transparent ">Relive Your Past Experiences </p>
                </div>
              </div>
            </div>
      </div>
