...
const DeleteEvent: React.FC<DeleteEventProps> = ({ eve, onComplete }) => {
  ...
  const handleSubmit = async () => {
    try {
      const response = await axios.delete(`http://localhost:8000/api/deleteEvent/${idEvent}`, {
        headers: {
  ...
  return (
    <div>
      <button onClick={() => togglePopup(eve.id)} className="font-medium text-red-600 dark:text-red-500 hover:underline">
        Delete
      </button>
          ...
          <div className="relative p-4 w-full max-w-md max-h-full">
            <div className="relative rounded-lg shadow bg-gray-900">
              <button
                ...
                onClick={() => setDeleteEventPopup(!deleteEventPopup)}
              >
